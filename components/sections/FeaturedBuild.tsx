  ...
  return (
    <section className="w-full overflow-hidden rounded-xl border border-dashed border-gray-300 bg-white shadow-sm dark:border-gray-700 dark:bg-ink">
      <div className="grid gap-0 sm:grid-cols-[300px_1fr] sm:items-stretch">
        <a href={featuredBuild.href} target="_blank" rel="noopener noreferrer" className="bg-gray-100 dark:bg-black/30 overflow-hidden flex items-center justify-center group block">
          {hasImg ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={isExternal ? featuredBuild.image : featuredBuild.image} alt={featuredBuild.title} className="h-40 w-full object-cover sm:h-full group-hover:scale-105 transition-transform duration-500" />
          ) : (
